...
const EMPTY: Omit<LunchItem, 'id'> = {
  name: '', emoji: '🍱', imageUrl: '', searchTerm: '', subtitle: 'Today\'s Special', order: 0,
};
...
export default function LunchSpecials() {
  ...
  const openEdit = (item: LunchItem) => { setEditing(item); setForm({ name: item.name, emoji: item.emoji, imageUrl: item.imageUrl, searchTerm: item.searchTerm, subtitle: item.subtitle, order: item.order }); setModal(true); };
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await updateDoc(doc(db, 'lunchSpecials', editing.id), { ...form });
        toast.success('Updated!');
      } else {
        await addDoc(collection(db, 'lunchSpecials'), { ...form });
        toast.success('Added!');
      }
      setModal(false);
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white">Lunch Specials</h1>
          <p className="text-sm text-gray-500 mt-0.5">These appear as the "Lunch Specials" section on the home screen.</p>
        </div>
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 rounded-2xl p-4 text-sm text-blue-700 dark:text-blue-300 font-medium">
        <strong>How it works:</strong> Add ≥ 4 items here and they appear as a horizontal scroll section on the home screen under "Lunch Specials 🍱". Each item has an image and tapping it searches for that food.
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="font-black text-gray-800 dark:text-white">No lunch specials yet</p>
          <p className="text-sm text-gray-500 mt-1">Click "Load Defaults" to get started or add manually.</p>
        ...
        <div className="space-y-2">
          {items.map(item => (
            <motion.div key={item.id} layout
              ...
              <div className="w-14 h-14 rounded-xl overflow-hidden bg-amber-50 flex-shrink-0 flex items-center justify-center">
                {item.imageUrl
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{item.subtitle} · Search: "{item.searchTerm || item.name}"</p>
                <p className="text-xs text-gray-300 mt-0.5">Order: {item.order}</p>
              </div>
        ...
        {modal && (
          <motion.div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/40 backdrop-blur-sm"
            ...
            <motion.div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-3xl p-6 space-y-4 shadow-2xl"
              ...
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-black">{editing ? 'Edit Item' : 'Add Lunch Special'}</h2>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input value={form.subtitle} onChange={e => F('subtitle', e.target.value)} placeholder="e.g. Afternoon Special"
                    className="w-full mt-1 p-2.5 border rounded-xl text-sm dark:bg-gray-800 dark:border-gray-700" />
                </div>
