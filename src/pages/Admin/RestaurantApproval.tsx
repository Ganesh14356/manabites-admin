...
import {
  ...
  ShieldCheck, ShieldX, Building2, Edit2, Save, Camera, Link as LinkIcon,
} from 'lucide-react';
...
export default function RestaurantApproval() {
  ...
  // Clipboard paste support for logo when edit form is open
  useEffect(() => {
    if (!isEditing) return;
  ...
  const handleSaveEdit = async () => {
    ...
    let logo = detailTarget.logo || '';
    if (logoFile) {
      setUploadingLogo(true);
      try {
        logo = await uploadToImgBB(logoFile);
      } catch {
                ...
                <section>
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      {/* Logo image upload */}
                      <div>
                        <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Restaurant Logo / Photo</label>
                        <label className="cursor-pointer block">
                          <div className={`w-full h-32 border-2 border-dashed rounded-xl flex items-center justify-center overflow-hidden transition-colors ${logoPreview ? 'border-transparent' : 'border-gray-200 hover:border-brand'}`}>
                            {logoPreview ? (
                              <img src={logoPreview} className="w-full h-full object-cover rounded-xl" alt="logo preview" />
                            ) : (
                              <div className="text-center">
                              ...
                              <input
                                ...
                                onKeyDown={e => {
                                  if (e.key === 'Enter') {
                                    ...
                                    if (url) { setLogoPreview(url); setLogoFile(null); setShowLogoUrl(false); setLogoUrlInput(''); }
                                  }
                                }}
                                placeholder="https://example.com/logo.jpg"
                                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs outline-none focus:border-brand transition-colors flex-1"
                                autoFocus
                              />
                              <button type="button" disabled={!logoUrlInput.trim()} onClick={() => {
                                const url = logoUrlInput.trim();
                                if (url) { setLogoPreview(url); setLogoFile(null); setShowLogoUrl(false); setLogoUrlInput(''); }
                              }} className="px-3 py-2 bg-brand text-white text-xs font-bold rounded-lg disabled:opacity-40">Use</button>
                              <button type="button" onClick={() => { setShowLogoUrl(false); setLogoUrlInput(''); }} className="px-2 py-2 bg-gray-100 text-gray-500 rounded-lg">
                        ...
                        {logoPreview && (
                          <button type="button" onClick={() => { setLogoFile(null); setLogoPreview(null); }} className="mt-1.5 text-xs text-red-400 hover:text-red-600 flex items-center gap-1">
